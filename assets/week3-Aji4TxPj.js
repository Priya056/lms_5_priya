...
Local search algorithms explore neighbors seeking improvement. Hill climbing moves to steepest uphill neighbor but gets stuck in local optima. Simulated annealing accepts worse solutions with decreasing probability to escape local optima. Constraint satisfaction problems use backtracking with pruning (arc consistency) and smart variable/value ordering. Linear programming optimizes linear objectives under linear constraints. These techniques solve diverse optimization problems from scheduling to routing.`,r=[{question:`What is the main limitation of hill climbing?`,options:[`A. Too slow`,`B. Gets stuck in local optima`,`C. Uses too much memory`,`D. Can't handle complex problems`],correct:1},{question:`How does simulated annealing escape local optima?`,options:[`A. Always moves uphill`,`B. Accepts worse solutions with decreasing probability`,`C. Random restart`,`D. Uses multiple climbers`],correct:1},{question:`What does the AC-3 algorithm do?`,options:[`A. Finds optimal tour`,`B. Enforces arc consistency in CSP`,`C. Calculates heuristics`,`D. Schedules variables`],correct:1},{question:`What does MRV heuristic select?`,options:[`A. Value with most constraints`,`B. Variable with smallest domain`,`C. Variable with most constraints`,`D. Most recent variable`],correct:1}],i=[{front:`What is the main advantage and disadvantage of hill climbing?`,back:`Advantage: Simple and fast, often finds good solutions quickly. Disadvantage: Gets stuck in local optima and cannot escape to find global optima. It only considers neighbors and stops when no uphill move exists.`},{front:`How does simulated annealing enable escape from local optima?`,back:`Simulated annealing accepts moves to worse solutions with probability proportional to e^(ΔE/T), where T is temperature. As temperature decreases over time, the probability of accepting bad moves decreases, allowing escape from local optima early on.`},{front:`What are the three components of a Constraint Satisfaction Problem (CSP)?`,back:`1) Variables: X1, X2, ..., Xn to be assigned values. 2) Domains: D1, D2, ..., Dn specifying possible values for each variable. 3) Constraints: Restrictions on which value combinations are allowed.`},{front:`What is arc consistency (AC) and why is it useful?`,back:`Arc consistency (Xi, Xj) means every value of Xi has a consistent value in Xj satisfying the constraint between them. Enforcing arc consistency before backtracking search reduces domain sizes and prunes impossible branches, improving efficiency.`},{front:`What does the Minimum Remaining Values (MRV) heuristic do?`,back:`MRV selects the unassigned variable with the smallest domain. This heuristic reduces branching factor early by constraining the most restrictive variables first, often leading to faster solutions.`},{front:`What is the difference between hill climbing and steepest ascent?`,back:`Hill climbing moves to the first improving neighbor found. Steepest ascent evaluates all neighbors and moves to the best one. Steepest ascent is more likely to escape plateaus and reach better optima but is slower.`},{front:`How does backtracking search work in CSPs?`,back:`Backtracking search assigns values to variables one at a time, checking constraints. If a conflict is found, it backtracks and tries a different value. Pruning (arc consistency) reduces the search space significantly.`},{front:`What is linear programming used for in optimization?`,back:`Linear programming optimizes a linear objective function subject to linear constraints. It's used for problems like maximizing profit or minimizing cost under resource constraints, solved efficiently with algorithms like simplex.`}],a=`Week 3 covered optimization techniques for finding best solutions. Local search algorithms (hill climbing, simulated annealing) explore neighbors, with simulated annealing accepting worse solutions to escape local optima. Constraint satisfaction problems use backtracking with arc consistency and smart ordering heuristics. Linear programming optimizes linear objectives under linear constraints. These techniques solve diverse problems from scheduling to routing to traveling salesman problem.`,o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};