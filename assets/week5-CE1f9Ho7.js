...
Neural networks combine weighted inputs with non-linear activations. Backpropagation computes gradients for learning. Gradient descent updates weights iteratively. CNNs excel at image processing through convolution and pooling. RNNs process sequences maintaining state, with LSTMs overcoming vanishing gradients. Proper loss functions, regularization (dropout, early stopping), and architecture design enable effective deep learning. Understanding these fundamentals is essential for modern AI applications.`,r=[{question:`What does a ReLU activation function do?`,options:[`A. Squashes to (0, 1)`,`B. Returns max(0, z)`,`C. Applies sigmoid`,`D. Normalizes input`],correct:1},{question:`What is backpropagation?`,options:[`A. Forward pass through network`,`B. Computing gradients of loss with respect to weights`,`C. Training data preprocessing`,`D. Model validation`],correct:1},{question:`What is the purpose of dropout in neural networks?`,options:[`A. Remove layers`,`B. Randomly deactivate neurons to prevent overfitting`,`C. Increase learning rate`,`D. Speed up training`],correct:1},{question:`What is an LSTM layer used for?`,options:[`A. Image processing`,`B. Processing sequences while addressing vanishing gradients`,`C. Classification only`,`D. Dimensionality reduction`],correct:1}],i=[{front:`What is the purpose of activation functions in neural networks?`,back:`Activation functions introduce non-linearity into neural networks, allowing them to learn complex patterns. Without activation functions, stacking layers would be equivalent to a single linear transformation. ReLU, sigmoid, and tanh are common choices.`},{front:`How does backpropagation compute gradients?`,back:`Backpropagation uses the chain rule to compute gradients of the loss with respect to each weight, propagating errors backward through the network. It calculates ∂L/∂w for each weight by composing partial derivatives from the output layer back to input.`},{front:`What are the three main variants of gradient descent?`,back:`Batch gradient descent uses all data to compute gradients (slow but stable). Stochastic gradient descent uses one sample per update (fast but noisy). Mini-batch gradient descent uses small batches (balance between speed and stability).`},{front:`How do Convolutional Neural Networks (CNNs) excel at image processing?`,back:`CNNs use convolutional layers with learned filters to detect local patterns, shared weights reduce parameters, and pooling layers reduce spatial dimensions. This architecture is efficient for images and captures hierarchical visual features.`},{front:`What problem do LSTM cells solve in RNNs?`,back:`LSTM cells solve the vanishing gradient problem in standard RNNs through gates (forget, input, output) that control information flow. The cell state acts as a memory, allowing gradients to flow over longer sequences without vanishing.`},{front:`What does dropout regularization do and why?`,back:`Dropout randomly deactivates neurons with probability p during training, forcing the network to learn redundant representations. This prevents co-adaptation and acts as an ensemble method, reducing overfitting significantly.`},{front:`What is the difference between cross-entropy and mean squared error loss?`,back:`MSE is for regression tasks, measuring squared differences: (1/n)Σ(yi - ŷi)². Cross-entropy is for classification, measuring probability distribution mismatch: -Σ yi log(ŷi). Cross-entropy typically works better for multi-class problems.`},{front:`What does the softmax activation function do?`,back:`Softmax converts raw output scores into a probability distribution over classes: softmax(zi) = e^zi / Σj e^zj. It ensures outputs sum to 1 and are between 0 and 1, making it ideal for multi-class classification output layers.`}],a=`Week 5 covered neural networks for deep learning. Artificial neurons combine weighted inputs with activation functions. Backpropagation computes gradients for efficient learning. Gradient descent updates weights iteratively. CNNs use convolution and pooling for image processing. RNNs process sequences maintaining state, with LSTMs overcoming vanishing gradient problems. Regularization techniques (dropout, early stopping) prevent overfitting. Understanding architectures and training procedures enables building effective deep learning models.`,o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};