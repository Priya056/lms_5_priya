...
Loops enable repetitive code execution. \`while\` loops repeat based on a condition, while \`for\` loops iterate through sequences. Lists store multiple values, with useful methods like append(), pop(), and sort(). The \`enumerate()\` function provides both index and value during iteration, and \`break\`/\`continue\` control loop flow.`,r=[{question:"How many times will this loop execute? `for i in range(5):`",options:[`A. 4 times`,`B. 5 times`,`C. 6 times`,`D. Infinite times`],correct:1},{question:"What does `fruits.pop()` do?",options:[`A. Adds an item to the list`,`B. Removes the first item`,`C. Removes and returns the last item`,`D. Prints the list`],correct:2},{question:"What will `list(range(1, 5, 2))` produce?",options:[`A. [1, 2, 3, 4, 5]`,`B. [1, 3]`,`C. [1, 3, 5]`,`D. [0, 2, 4]`],correct:2},{question:`In the enumerate() function, what is returned?`,options:[`A. Only the index`,`B. Only the value`,`C. Both index and value as a tuple`,`D. Neither index nor value`],correct:2}],i=[{front:`What is the difference between a while loop and a for loop?`,back:`while loops repeat based on a condition. for loops iterate a fixed number of times through a sequence. Use while for unknown iterations, for for known sequences.`},{front:`How many times does for i in range(5): execute?`,back:`5 times, with i taking values 0, 1, 2, 3, 4. range(5) generates numbers from 0 to 4 (not including 5).`},{front:`What does list.append() do?`,back:`Adds an item to the end of a list. Example: fruits.append('orange') adds 'orange' to the end of the fruits list.`},{front:`What does break do in a loop?`,back:`break immediately exits the loop. Useful for stopping early when a condition is met. Example: if i == 5: break stops the loop when i equals 5.`},{front:`What does continue do in a loop?`,back:`continue skips the rest of the current iteration and moves to the next one. Example: if i == 2: continue skips iteration when i equals 2.`},{front:`What does enumerate() return?`,back:`enumerate() returns both the index and value during iteration as a tuple. Example: for i, fruit in enumerate(fruits): gives you the position and fruit name.`},{front:`What does list.pop() do?`,back:`Removes and returns the last item from a list. Example: last = fruits.pop() removes and stores the last fruit. pop(0) removes the first item.`}],a="Week 2 introduced loops for repetition: `while` loops that continue based on conditions, and `for` loops that iterate through sequences. You learned the `range()` function for generating number sequences, list data structures with methods (append, pop, insert, remove, sort), and control statements (break, continue). The `enumerate()` function enables access to both index and value during iteration.",o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};