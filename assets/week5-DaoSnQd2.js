...
Unit tests ensure code quality and correctness. Test files follow naming conventions (\`test_*.py\`) and contain functions starting with \`test_\`. The \`assert\` statement verifies conditions, and \`pytest.raises()\` tests exception handling. Edge cases must be tested thoroughly, and parameterized tests reduce code duplication. Following best practices like the Arrange-Act-Assert pattern leads to reliable, maintainable tests.`,r=[{question:`What naming convention should test files follow?`,options:[`A. file_test.py`,`B. test_file.py or file_test.py`,`C. testing_file.py`,`D. _test_file.py`],correct:1},{question:`How do you verify that an exception is raised in pytest?`,options:[`A. try/except`,`B. pytest.raises()`,`C. pytest.error()`,`D. assert raises()`],correct:1},{question:"What does the `assert` statement do when the condition is False?",options:[`A. Continues execution`,`B. Prints a warning`,`C. Raises AssertionError`,`D. Exits the program`],correct:2},{question:`Which pattern is recommended for structuring tests?`,options:[`A. Act-Arrange-Assert`,`B. Assert-Arrange-Act`,`C. Arrange-Act-Assert`,`D. Test-Run-Verify`],correct:2}],i=[{front:`What does the assert statement do?`,back:`The assert statement tests a condition and raises AssertionError if the condition is False. Example: assert x > 0 raises AssertionError if x is not greater than 0.`},{front:`What naming convention should test files follow?`,back:`Test files should be named test_*.py or *_test.py (e.g., test_calculator.py or calculator_test.py) so pytest can discover them automatically.`},{front:`How do you test that an exception is raised in pytest?`,back:`Use pytest.raises() context manager: with pytest.raises(ZeroDivisionError): divide(10, 0). This verifies the exception is raised during the code execution.`},{front:`What should test function names start with?`,back:`Test function names should start with test_ (e.g., test_add, test_divide_by_zero). This naming convention allows pytest to automatically discover and run the tests.`},{front:`Describe the Arrange-Act-Assert pattern.`,back:`Arrange: Set up test data and preconditions. Act: Execute the code being tested. Assert: Check that the results are correct. This pattern makes tests clear and maintainable.`},{front:`What is a parameterized test?`,back:`A parameterized test uses @pytest.mark.parametrize to run one test function with multiple sets of inputs, reducing code duplication. Example: @pytest.mark.parametrize("x,y,expected", [(1,2,3), (4,5,9)])`},{front:`When should you test edge cases?`,back:`Always test edge cases like zero, negative numbers, empty values, and boundary conditions. Edge cases often reveal bugs that normal inputs miss.`},{front:`Should you test built-in Python functions?`,back:`No, you should not test standard library functions. Trust that they work correctly. Focus on testing your own code to ensure it works as intended.`}],a="Week 5 covered unit testing with pytest to ensure code quality. Test files follow naming conventions (`test_*.py`), and test functions start with `test_`. The `assert` statement verifies conditions, while `pytest.raises()` tests exception handling. Parameterized tests handle multiple scenarios efficiently. Best practices include testing edge cases, following Arrange-Act-Assert pattern, and using descriptive test names.",o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};