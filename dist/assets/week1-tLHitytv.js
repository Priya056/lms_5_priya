...
Knowledge representation uses propositional and predicate logic to formalize facts and rules. Model checking verifies inference by examining truth assignments. Inference rules like modus ponens derive new knowledge. CNF provides standard form for automated reasoning. Resolution is a powerful inference rule for CNF. DPLL efficiently checks satisfiability using unit propagation and pure literal elimination. Knowledge bases implement forward and backward chaining for query answering.`,r=[{question:`What does it mean for a KB to entail a proposition?`,options:[`A. The KB contains the proposition`,`B. The proposition is true in all models where KB is true`,`C. The proposition contradicts the KB`,`D. The proposition is a rule in the KB`],correct:1},{question:`What is the result of: 'If it rains, the ground is wet' AND 'It rains'?`,options:[`A. It doesn't rain`,`B. The ground is not wet`,`C. The ground is wet`,`D. Undetermined`],correct:2},{question:`What is the standard form for automated reasoning in logic?`,options:[`A. Disjunctive Normal Form`,`B. Conjunctive Normal Form`,`C. Negation Normal Form`,`D. Predicate Normal Form`],correct:1},{question:`What technique does DPLL use to simplify satisfiability checking?`,options:[`A. Model enumeration`,`B. Unit propagation and pure literal elimination`,`C. Random search`,`D. Depth-first traversal`],correct:1}],i=[{front:`What are the five logical operators in propositional logic?`,back:`Conjunction (AND, ∧), Disjunction (OR, ∨), Negation (NOT, ¬), Implication (→), and Biconditional (↔). These operators combine propositions to form complex logical statements.`},{front:`What is the difference between satisfiable and valid formulas?`,back:`A formula is satisfiable if it's true in at least one model, and valid if it's true in all models. Valid formulas are always satisfiable, but satisfiable formulas may not be valid.`},{front:`What is modus ponens and why is it important?`,back:`Modus ponens states: from P → Q and P, we can infer Q. It's a fundamental inference rule that allows us to derive new knowledge from existing facts and rules in a knowledge base.`},{front:`Why is Conjunctive Normal Form (CNF) important for automated reasoning?`,back:`CNF is a standard form where formulas are conjunctions of disjunctions. It enables efficient automated reasoning algorithms like resolution and DPLL to check satisfiability and perform inference.`},{front:`How does the resolution rule work?`,back:`Resolution: from (P ∨ α) and (¬P ∨ β), infer (α ∨ β). This rule eliminates a variable P from two clauses. Resolution is complete for refutation, meaning it can prove any valid formula by finding a contradiction.`},{front:`What is unit propagation in the DPLL algorithm?`,back:`Unit propagation identifies clauses with only one unassigned literal (unit clauses) and immediately assigns that literal to satisfy the clause. This simplification reduces the search space significantly.`},{front:`How does a knowledge base help with reasoning?`,back:`A knowledge base stores facts and rules, allowing systems to ask queries and derive answers. It can use forward chaining (deriving conclusions from facts) or backward chaining (working backward from goals) to answer questions.`},{front:`What is the difference between first-order logic and propositional logic?`,back:`First-order logic extends propositional logic with quantifiers (∀ and ∃) and predicates, allowing reasoning about objects and their properties. Propositional logic only deals with true/false propositions.`}],a=`Week 1 covered knowledge representation using propositional logic with operators (AND, OR, NOT, implication, biconditional). Model checking verifies KB entailment by examining truth assignments. Inference rules like modus ponens derive new knowledge. CNF provides standard form for automated reasoning. Resolution infers new clauses from CNF formulas. DPLL efficiently checks satisfiability using unit propagation and pure literal elimination. Knowledge bases implement query answering for logical inference.`,o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};