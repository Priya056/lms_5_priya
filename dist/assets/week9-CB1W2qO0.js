...
Advanced Python concepts include sets for unique collections and frozensets for immutability. Global variables and constants organize program state. Lambda functions provide inline anonymous functions. List/dict comprehensions offer concise syntax for transformations. Type hints document expected types. Docstrings explain functions and classes. The argparse module handles command-line arguments. Unpacking assigns sequences to variables. Generators yield values efficiently. These tools make Python code more concise and maintainable.`,r=[{question:`What is the difference between a set and a frozenset?`,options:[`A. No difference`,`B. Sets are mutable, frozensets are immutable`,`C. Sets store tuples, frozensets store lists`,`D. Frozensets are faster`],correct:1},{question:`What does a list comprehension do?`,options:[`A. Compresses lists`,`B. Creates a list using concise syntax`,`C. Counts items in a list`,`D. Sorts a list`],correct:1},{question:`What is a lambda function?`,options:[`A. A class method`,`B. An anonymous one-line function`,`C. A type of variable`,`D. A built-in error`],correct:1},{question:`What keyword allows modifying global variables inside a function?`,options:[`A. extern`,`B. public`,`C. global`,`D. nonlocal`],correct:2}],i=[{front:`What's the difference between a set and a frozenset?`,back:`Sets are mutable (can add/remove elements). Frozensets are immutable (cannot be changed). Example: s = {1,2,3}, s.add(4) works. f = frozenset({1,2,3}), f.add(4) raises error.`},{front:`What does the global keyword do?`,back:`global allows you to modify a global variable inside a function. Example: global counter; counter += 1 modifies the global counter, not a local variable.`},{front:`What is a lambda function?`,back:`A lambda is an anonymous one-line function. Syntax: lambda x: x ** 2. Useful with map/filter. Example: squares = map(lambda x: x**2, [1,2,3])`},{front:`How do you use list comprehensions?`,back:`Syntax: [expression for item in iterable if condition]. Example: [x**2 for x in range(5)] creates [0,1,4,9,16]. More concise than loop+append.`},{front:`What is a type hint and why use it?`,back:`Type hints annotate expected types for clarity and IDE support. Example: def add(x: int, y: int) -> int: return x + y. Doesn't enforce types at runtime but helps catch errors.`},{front:`What is argparse used for?`,back:`argparse parses command-line arguments. Example: parser.add_argument('name') lets you run script.py John. Handles flags like -v or --verbose.`},{front:`What do *args and **kwargs mean?`,back:`*args captures variable positional arguments as a tuple. **kwargs captures variable keyword arguments as a dictionary. Example: def func(*args, **kwargs): processes both.`},{front:`How do generators work and why use them?`,back:`Generators yield values one at a time using yield keyword. Memory-efficient for large sequences. Example: def count(n): while n > 0: yield n; n -= 1`}],a=`Week 9 covered advanced Python concepts: sets for unique collections and frozensets for immutability, global variables and constants for program state, lambda functions for inline anonymous operations, list and dictionary comprehensions for concise transformations, type hints for documenting expected types, docstrings for documentation, argparse for command-line parsing, unpacking for assigning sequences, and generators for memory-efficient iteration. These tools enable more expressive and maintainable Python code.`,o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};