...
Libraries extend Python's functionality. The \`import\` statement loads modules, with options for specific imports and aliases. Python's standard library includes random, sys, statistics, and datetime for common tasks. Third-party libraries like requests extend capabilities for web APIs. Understanding how to use and create modules is essential for building complex applications.`,r=[{question:`How do you import a specific function from a module?`,options:[`A. import math.sqrt`,`B. from math import sqrt`,`C. import sqrt from math`,`D. using math.sqrt`],correct:1},{question:"What does `random.randint(1, 10)` do?",options:[`A. Returns a random float`,`B. Returns a random integer between 1 and 10`,`C. Returns a random number from 1 to 9`,`D. Shuffles a list`],correct:1},{question:`How do you install a third-party library?`,options:[`A. import pip library_name`,`B. pip install library_name`,`C. python install library_name`,`D. library_name.install()`],correct:1},{question:`Which module would you use to get the current date and time?`,options:[`A. time`,`B. clock`,`C. datetime`,`D. now`],correct:2}],i=[{front:`What is the difference between import random and from random import randint?`,back:`import random loads the entire module. from random import randint imports only randint. The latter is more specific and efficient if you only need one function.`},{front:`What does random.randint(1, 10) do?`,back:`Returns a random integer between 1 and 10 (inclusive). Example: random.randint(1, 100) gives a random number from 1 to 100.`},{front:`How do you install a third-party library?`,back:`Use pip (Python package installer) from the command line. Example: pip install requests installs the requests library.`},{front:`What module would you use to get the current date and time?`,back:`The datetime module. Example: from datetime import datetime; now = datetime.now() gets the current date and time.`},{front:`What does the statistics module provide?`,back:`Functions to calculate statistical measures: mean (average), median (middle value), and mode (most frequent value) of a dataset.`},{front:`What does sys.argv contain?`,back:`sys.argv is a list of command-line arguments. sys.argv[0] is the script name, and subsequent elements are arguments passed to the script.`},{front:`What is the purpose of aliasing in imports?`,back:`Aliasing gives a module a shorter or different name. Example: import numpy as np allows you to use np instead of numpy, making code more concise.`},{front:`What does the if __name__ == '__main__': pattern do?`,back:`This checks if the script is run directly (not imported as a module). Code under it only executes when the script is run directly, not when imported.`}],a=`Week 4 introduced libraries as reusable code collections. You learned import syntax (basic import, from X import Y, aliasing), used standard library modules (random, sys, statistics, datetime) for common tasks, and explored third-party libraries (cowsay, emoji, requests) installed via pip. Creating your own modules allows organizing code into reusable files.`,o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};