...
NLP processes text through tokenization and representation. Bag-of-words and TF-IDF provide simple representations. Word embeddings (Word2Vec, GloVe) capture semantic meaning in dense vectors. Transformers revolutionized NLP through self-attention mechanism. BERT provides bidirectional pre-trained representations. GPT enables text generation. Attention mechanisms weight importance of different words. Pre-trained models from libraries like Hugging Face enable practical NLP tasks from classification to translation.`,r=[{question:`What is tokenization in NLP?`,options:[`A. Converting text to lowercase`,`B. Breaking text into individual tokens (words, subwords)`,`C. Removing punctuation`,`D. Counting word frequency`],correct:1},{question:`What is the main benefit of word embeddings over bag-of-words?`,options:[`A. Smaller file size`,`B. Captures semantic meaning in continuous space`,`C. Faster training`,`D. No preprocessing needed`],correct:1},{question:`What is the key innovation of the Transformer architecture?`,options:[`A. Recurrent connections`,`B. Self-attention mechanism`,`C. Convolutional layers`,`D. Dropout regularization`],correct:1},{question:`What is the difference between BERT and GPT?`,options:[`A. BERT is faster`,`B. BERT is bidirectional, GPT is unidirectional`,`C. GPT is for classification only`,`D. No significant difference`],correct:1}],i=[{front:`What is the purpose of tokenization in NLP?`,back:`Tokenization breaks raw text into meaningful units (words, subwords, or characters) that can be processed by NLP models. It's the first preprocessing step that converts continuous text into discrete, analyzable elements.`},{front:`What are N-grams and how are they used in language modeling?`,back:`N-grams are sequences of n consecutive tokens. Bigrams (n=2) are pairs, trigrams (n=3) are triplets. N-gram language models predict the next word based on previous n-1 words, useful for simple baseline models.`},{front:`How does TF-IDF weight words differently from bag-of-words?`,back:`Bag-of-words counts word frequencies equally. TF-IDF weights words by importance: TF-IDF = TF * IDF where IDF = log(N / num_docs_with_term). Common words across documents get low weights, unique/meaningful words get high weights.`},{front:`What makes Word2Vec embeddings effective?`,back:`Word2Vec learns dense word vectors through skip-gram (predicting context from target) or CBOW (predicting target from context). Similar words end up close in embedding space, and vector arithmetic captures semantic relationships (e.g., king - man + woman ≈ queen).`},{front:`What is the self-attention mechanism in Transformers?`,back:`Self-attention computes a weighted sum of values based on query-key similarity: Attention(Q,K,V) = softmax(QK^T/√d)V. Each position attends to all other positions, allowing the model to capture long-range dependencies efficiently.`},{front:`How does BERT pre-training differ from GPT?`,back:`BERT uses bidirectional pre-training with masked language modeling (masking random words and predicting them). GPT uses unidirectional pre-training predicting next tokens. BERT is better for understanding/classification, GPT is better for generation.`},{front:`What are the main tasks BERT can be fine-tuned for?`,back:`Classification (sentiment analysis, topic classification), question answering, named entity recognition, and semantic similarity. BERT's bidirectional representations transfer well to many downstream NLP tasks.`},{front:`What does an attention head in a multi-head attention layer do?`,back:`Each attention head learns a different way to relate positions to each other through different Q, K, V projections. Multi-head attention concatenates outputs from all heads, allowing the model to attend to different semantic subspaces simultaneously.`}],a=`Week 6 covered Natural Language Processing for understanding language. Tokenization breaks text into tokens. Bag-of-words and TF-IDF provide simple representations. Word embeddings (Word2Vec, GloVe) capture semantic meaning. Transformers use self-attention for powerful representations. BERT provides bidirectional pre-training. GPT enables text generation. These techniques power applications from sentiment analysis to machine translation to dialogue systems.`,o={title:e,source:t,content:n,quiz:r,flashcards:i,summary:a};export{n as content,o as default,i as flashcards,r as quiz,t as source,a as summary,e as title};